# googlescrape.py
# This module defines the GoogleDriveCrawler class, which provides functionality to scrape text content from a specified Google Drive folder.
# It handles various file types (Google Docs, PDFs, Word documents, CSVs) and produces a structured JSON output containing the extracted text 
# and metadata for each document. The crawler also supports caching results to avoid redundant API calls.
import io
import json
import os
import re
...
class GoogleDriveCrawler:
	def __init__(self, cache_path: str = DEFAULT_DRIVE_OUTPUT):
	...
	def resolve_folder_id(self, cli_folder_id: Optional[str]) -> str:
		if cli_folder_id:
		...
		parsed_id = self.extract_folder_id_from_link(env_folder_link)
		if parsed_id:
			return parsed_id
	...
	def scrape(self, folder_id: Optional[str] = None) -> Dict:
		if os.path.exists(self.cache_path):
			print(f"Found existing {self.cache_path}, loading from file instead of re-ingesting.")
			with open(self.cache_path, "r", encoding="utf-8") as file:
				return json.load(file)

		resolved_folder_id = self.resolve_folder_id(folder_id)
